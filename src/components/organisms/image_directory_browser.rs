use crate::{
    components::{
        atoms::label::Label,
        organisms::blob_image_select::{
            BlobBrowserDialog, BlobBrowserDialogConfig, IMAGE_BROWSER_ID,
        },
    },
            ...
            <div class={"w-full"}>
                <div id={IMAGE_BROWSER_ID} tabindex={"0"} class={dropdown_class}>
                    ...
                    <div tabindex={"0"} class={"dropdown-content w-full flex flex-col mb-4 gap-1 z-10"}>
                        <BlobBrowserDialog {config} ondatachanged={Callback::noop()} {onforceopenchanged} />
                    </div>
                </div>
            </div>
        </div>
